...
// #MARK - Constructors
    ...
    public MagnificentChart(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            int backgroundColor = typedArray.getColor(R.styleable.MagnificentChart_bg, Color.parseColor("#FFFFFF"));
...
//        int screenHeight = size.y;
    ...
    public void setChartItemsList(List<MagnificentChartItem> itemsList,int total){
        this.chartItemsList = itemsList;
        this.total = total;
        invalidate();
    }
...
// #MARK - Drawing Methods
    ...
    private void drawInsideCircle(Canvas canvas, Paint insideShadowPaint, Paint insideChartPaint){
        if(!isRound){
        	int radius = width/3 - 50;
        	int x = width/2;
        	int y = height/2;
        	
//            if(isShadowShowing){
//                canvas.drawCircle(width/2, height/2, width/3 - 30, insideShadowPaint);
//            }
            insideChartPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            insideChartPaint.setColor(Color.WHITE);
            canvas.drawCircle(x, y, radius, insideChartPaint);
            
            insideChartPaint.setAntiAlias(true);
            insideChartPaint.setTextSize(context.getResources().getDimension(R.dimen.text_size_31));
            insideChartPaint.setTextAlign(Align.CENTER);
            insideChartPaint.setColor(Color.RED);
            canvas.drawText(""+total, x, y, insideChartPaint);
            
            insideChartPaint.setAntiAlias(true);
            insideChartPaint.setTextSize(getResources().getDimension(R.dimen.text_size_12));
            insideChartPaint.setTextAlign(Align.CENTER);
            insideChartPaint.setColor(Color.GRAY);
            canvas.drawText("参与接单人", x, y+context.getResources().getDimension(R.dimen.text_size_15), insideChartPaint);
